...
class SynthesisVerificationPipeline:
    ...
    def _correct_tokens(
        ...
        Returns:
            Dict mapping key → corrected token indices [B]
        """
        ...
        if n_mismatched == 0:
            return generated  # Perfect match, nothing to correct

        logger.debug(
            f"  Correction: {n_mismatched}/{n_total} tokens mismatched "
            f"({n_mismatched / max(n_total, 1) * 100:.1f}%)"
        )
    ...
    def _explore_step(self) -> Dict[str, float]:
        ...
        if self.config.resampling is not None:
            ...
            corrected_tokens = self._correct_tokens(generated_tokens, retokenized)
