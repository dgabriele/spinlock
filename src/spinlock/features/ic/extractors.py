...
from .cnn_encoder import ICCNNEncoder, ICVAE
...
class ICExtractor:
    ...
    def __init__(self, config: ICConfig, device: torch.device):
        """
        ...
        if config.cnn.enabled:
            if config.cnn.use_vae:
                ...
                self.cnn_encoder = ICVAE(embedding_dim=config.cnn.embedding_dim)
            else:
                # Analysis mode: Encoder only
                self.cnn_encoder = ICCNNEncoder(embedding_dim=config.cnn.embedding_dim)
    ...
    def _build_registry(self) -> FeatureRegistry:
        ...
        """
        registry = FeatureRegistry(family_name="ic")
    ...
    def extract_all(
        ...
        if self.cnn_encoder is not None:
            ...
            # Extract features
            if isinstance(self.cnn_encoder, ICVAE):
    ...
    def generate_ics(
        ...
        """
        if not isinstance(self.cnn_encoder, ICVAE):
            raise ValueError(
    ...
    def is_generative(self) -> bool:
        ...
        return isinstance(self.cnn_encoder, ICVAE)
