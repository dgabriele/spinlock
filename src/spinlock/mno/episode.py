...
class EpisodeRunner:
    ...
    def __init__(
        ...
        Args:
            ...
            vqvae: Trained VQTokenizer for behavioral tokenization
            early_stopping: Early stopping criterion
            device: Device for computation
        """
        self.mno = mno.to(device)
        self.vqvae = vqvae
        # Move VQTokenizer's model to device
        self.vqvae.model = self.vqvae.model.to(device)
    ...
    def run_episode(
        ...
        Args:
            ...
            extract_features_fn: Optional function to extract features for VQ-VAE
                                 If None, use identity (assumes features already extracted)
        ...
        with torch.no_grad():
            for t in range(max_steps if max_steps is not None else 10000):
                ...
                # VQ-VAE encoding
                # TODO: Update extract_features_fn to provide features in VQTokenizer format
                # For now, assume features is a dict with temporal_features/initial_manual/initial_raw
                if isinstance(features, dict):
                    # New format: features dict compatible with VQTokenizer
                    tokens_dict = self.vqvae.tokenize(**features)
                    # Flatten tokens dict to single tensor
                    tokens = torch.cat([tokens_dict[k] for k in sorted(tokens_dict.keys())])
                else:
                    # Old format: single tensor (legacy)
                    # This path won't work with VQTokenizer - user must update extract_features_fn
                    raise NotImplementedError(
                        "EpisodeRunner now requires extract_features_fn to return a dict "
                        "compatible with VQTokenizer.tokenize():\n"
                        "  {'temporal_features': Tensor, 'initial_manual': Tensor, ...}\n"
                        "\n"
                        "V1 VQ-VAE has been removed. Please update your feature extraction."
                    )
