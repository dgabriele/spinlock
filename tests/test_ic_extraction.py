...
from spinlock.features.ic.config import ICConfig, ICManualConfig, ICCNNConfig
from spinlock.features.ic.manual_extractors import ICManualExtractor
from spinlock.features.ic.cnn_encoder import ICCNNEncoder, ICVAE, ICCNNDecoder
...
class TestHybridExtractor:
    """Tests for ICExtractor (hybrid manual + CNN)."""
    ...
    def test_hybrid_extractor_shape(self, device, synthetic_ics, ic_config):
        ...
        extractor = ICExtractor(config=ic_config, device=device)
    ...
    def test_hybrid_extractor_manual_only(self, device, synthetic_ics):
        ...
        extractor = ICExtractor(config=config, device=device)
    ...
    def test_vae_mode_generation(self, device, ic_config_vae):
        ...
        extractor = ICExtractor(config=ic_config_vae, device=device)
